...
class BasicBlock(layers.Layer):
    def __init__(self, filter_num, name, stride=1, **kwargs):
        ...
        self.stride = stride
        self.layers = []
        self.conv1 = layers.Conv3D(filter_num, (2, 3, 3), strides=(
            1, stride, stride), padding='same', name=name+'_1')
        # self.bn1=layers.BatchNormalization()
        self.relu = layers.Activation('relu')

        self.conv2 = layers.Conv3D(
            filter_num, (2, 3, 3), strides=1, padding='same', name=name+'_2')
        ...
        if stride != 1:
            ...
            self.downsample.add(layers.Conv3D(
                filter_num, (1, 1, 1), strides=(1, stride, stride)))
...
class Model:
    ...
    def load_model(self):
        ...
        if os.path.exists("./model/act_part.h5"):
            ...
            self.private_act_model = load_model(
                "./model/act_part.h5", custom_objects={'BasicBlock': BasicBlock})
        ...
        if os.path.exists("./model/move_part.h5"):
            ...
            self.private_move_model = load_model(
                "./model/move_part.h5", custom_objects={'BasicBlock': BasicBlock})
    ...
    def _build_model(self):
       ...
       # ------------------ build evaluate_net ------------------
        ...
        self.private_act_model.add(Conv3D(64, (2, 3, 3), strides=(
            1, 2, 2), input_shape=self.input_shape, name='conv1'))
        # self.private_act_model.add(BatchNormalization(name='b1'))
        self.private_act_model.add(Activation('relu'))
        self.private_act_model.add(MaxPooling3D(pool_size=(
            2, 2, 2), strides=1, padding="VALID", name='p1'))

        # resnet blocks
        self.private_act_model.add(self.build_resblock(64, 2, name='Resnet_1'))
        self.private_act_model.add(self.build_resblock(
            80, 2, name='Resnet_2', stride=2))
        self.private_act_model.add(self.build_resblock(
            128, 2, name='Resnet_3', stride=2))

        self.private_act_model.add(self.build_resblock(
            200, 2, name='Resnet_4', stride=2))
        ...
        self.private_act_model.add(Dense(
            self.act_dim, name="d1", kernel_regularizer=regularizers.l2(0.001)))        # action model
        ...
        self.private_move_model.add(Conv3D(64, (2, 3, 3), strides=(
            1, 2, 2), input_shape=self.input_shape, name='conv1'))
        # self.private_move_model.add(BatchNormalization(name='b1'))
        self.private_move_model.add(Activation('relu'))
        self.private_move_model.add(MaxPooling3D(pool_size=(
            2, 2, 2), strides=1, padding="VALID", name='p1'))

        # resnet blocks
        self.private_move_model.add(
            self.build_resblock(64, 2, name='Resnet_1'))
        self.private_move_model.add(
            self.build_resblock(80, 2, name='Resnet_2', stride=2))
        self.private_move_model.add(self.build_resblock(
            128, 2, name='Resnet_3', stride=2))
        self.private_move_model.add(self.build_resblock(
            200, 2, name='Resnet_4', stride=2))
